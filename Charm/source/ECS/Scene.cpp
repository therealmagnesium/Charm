...
namespace Charm::ECS
{
    ...
    namespace Scenes
    {
        ...
        void DrawMeshesAndLights(Scene& scene, Entity& selectionContext, bool isRuntime)
        {
            const auto ResolveMaterial = [](const MeshRendererComponent& meshRenderer, Model* model, u32 meshIndex) -> Material*
            {
                if (meshIndex < meshRenderer.materialOverrides.size())
                {
                    const AssetHandle overrideHandle = meshRenderer.materialOverrides[meshIndex];
                    if (AssetManager::IsHandleValid(overrideHandle))
                    {
                        Material* overrideMaterial = AssetManager::GetAsset<Material>(overrideHandle);
                        if (overrideMaterial != NULL)
                            return overrideMaterial;
                    }
                }

                const s32 materialIndex = model->meshes[meshIndex].materialIndex;
                if (materialIndex >= 0 && materialIndex < (s32)model->materials.size())
                    return &model->materials[materialIndex];

                return NULL;
            };

            // Group entities by (VAO, Material*) so that:
            // - Identical mesh + identical material → same group → single instanced draw call.
            // - Same mesh but different material override → separate groups → separate draw calls.
            // Using Material* as the key is safe because model default materials have stable
            // addresses inside model.materials, and AssetManager-owned materials have stable
            // addresses in the heap.
            using MeshKey = std::pair<u32, Material*>;

            struct MeshKeyHash
            {
                std::size_t operator()(const MeshKey& key) const
                {
                    const std::size_t h0 = std::hash<u32>()(key.first);
                    const std::size_t h1 = std::hash<Material*>()(key.second);
                    return h0 ^ (h1 << 1);
                }
            };
            ...
            for (const auto entityID : meshRenderers)
            {
                ...
                {
                    const u32 slot = (u32)meshRenderer.submeshIndex;
                    Mesh& mesh = model->meshes[slot];
                    Material* material = ResolveMaterial(meshRenderer, model, slot);
                    if (material == NULL) continue;

                    const MeshKey key = {mesh.vertexArray, material};
                ...
                else
                {
                    for (u32 i = 0; i < (u32)model->meshes.size(); i++)
                    {
                        ...
                        Material* material = ResolveMaterial(meshRenderer, model, i);
                        if (material == NULL) continue;
            ...
            // Materials own their shader reference; do not overwrite it here.
            // Model::ProcessMesh sets shader = &blinnPhongShader at load time.
            // Standalone .chmat assets set their shader when loaded.

...
bool ImGui::ButtonFramed(const char* label, const ImVec2& size, ImGuiButtonFlags flags)
{
    ...
    ImVec2 newSize = ImGui::CalcItemSize(ImVec2(size.x, size.y),
                                         labelSize.x + style.FramePadding.x * 2.f,
                                         labelSize.y + style.FramePadding.y * 2.f);

    const ImRect bb(position, position + newSize);
    ImGui::ItemSize(newSize, style.FramePadding.y);
    ...
    const ImU32 color = ImGui::GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered
                                                                                               : ImGuiCol_Button);
    ImGui::RenderNavCursor(bb, id);
    ImGui::RenderFrameLines(bb.Min, bb.Max, color, style.FrameRounding, 4.f);
...
bool ImGui::ImageButtonEmpty(const char* str_id, const ImVec2& image_size, float thickness, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    RenderNavCursor(bb, id);
    window->DrawList->AddRect(bb.Min + padding, bb.Max - padding, color, 0.f, 0, thickness);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        if (IsKeyPressed(ImGuiKey_LeftArrow)) { state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINESTART : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT
                                                                                                                                            : STB_TEXTEDIT_K_LEFT) |
                                                                    k_mask); }
        else if (IsKeyPressed(ImGuiKey_RightArrow)) { state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINEEND : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDRIGHT
